...
r"""\
ErpBizz HTTP layer / WSGI application
...
Application.__call__
  ...
  request and itself in an ErpBizz http request. The ErpBizz http request is
...
ErpBizz URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.odoo.com/documentation/master/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through ErpBizz via py-QWeb form, embed a CSRF
...
<p>Please verify the hostname, <a href=/erpbizz/login>login</a> and try again.</p>

<!-- Alternatively, use the X-ErpBizz-Database header. -->
"""
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, ErpBizz will
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are ErpBizz addons.
        """
...
class Request:
    ...
    def _get_session_and_dbname(self):
        ...
        header_dbname = self.httprequest.headers.get('X-ErpBizz-Database')
...
class HttpDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, SessionExpiredException):
            ...
            response = self.request.redirect_query('/erpbizz/login', {'redirect': self.request.httprequest.full_path})
            if was_connected:
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "ErpBizz Session Expired"
            ...
            try:
                ...
                if self.get_static_file(httprequest.path):
                    ...
                    try:
                        ...
                        if (httprequest.path.startswith('/erpbizz/')
                            or httprequest.path in (
                                '/erpbizz', '/web', '/web/login', '/erpbizz/login', '/test_http/ensure_db',
                            )):
