...
class BaseTestUi(AccountTestMockOnlineSyncCommon):
    ...
    def main_flow_tour(self):
        ...
        self.start_tour("/erpbizz", 'main_flow_tour', login="admin", timeout=180)
...
class TestUi(BaseTestUi):
    ...
    def test_company_switch_access_error(self):
        ...
        with mute_logger("odoo.http"):
            self.start_tour(f"/erpbizz/action-{act_window.id}", "test_company_switch_access_error", login="admin", cookies={"cids": f"{company1.id}-{company2.id}"})
    ...
    def test_company_access_error_redirect(self):
        ...
        with mute_logger("odoo.http"):
            self.start_tour(f"/erpbizz/action-{act_window.id}/{record_p2.id}", "test_company_access_error_redirect", login="admin", cookies={"cids": f"{company1.id}"})
    ...
    def test_company_switch_access_error_debug(self):
        ...
        with mute_logger("odoo.http"):
            self.start_tour(f"/erpbizz/action-{act_window.id}?debug=assets", "test_company_switch_access_error", login="admin", cookies={"cids": current_companies})
