...
def standalone(*tags):
    ...
    ``tags`` and the corresponding ErpBizz module name.
    """
    def register(func):
...
class BaseCase(case.TestCase):
    """ Subclass of TestCase for ErpBizz-specific code. This class is abstract and
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/erpbizz/addons/" in filename:
                        filename = filename.rsplit("/erpbizz/addons/", 1)[1]
                    if count > expected:
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way ErpBizz widgets are created they all appear as Class(...)
...
class freeze_time:
    """ Object to replace the freezegun in ErpBizz test suites
