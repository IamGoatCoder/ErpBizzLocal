...
DOCKERUSER = """
RUN mkdir /var/lib/erpbizz && \
    groupadd -g %(group_id)s odoo && \
    useradd -u %(user_id)s -g odoo odoo -d /var/lib/erpbizz && \
    mkdir /data && \
    chown odoo:odoo /var/lib/erpbizz /data
USER odoo
...
class Docker():
    ...
    def test_odoo(self):
        logging.info('Starting to test ErpBizz install test')
        ...
        if self.is_running():
            self.stop()
            raise OdooTestTimeoutError('ErpBizz pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise OdooTestError('Error while installing/starting ErpBizz after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        os.rename(glob('%s/dist/erpbizz-*.tar.gz' % self.args.build_dir)[0], '%s/odoo_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/erpbizz-*.zip' % self.args.build_dir)[0], '%s/odoo_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su odoo -s /bin/bash -c "python3 -m venv /var/lib/erpbizz/odoovenv"',
            'su odoo -s /bin/bash -c "/var/lib/erpbizz/odoovenv/bin/python3 -m pip install --upgrade pip"',
            'su odoo -s /bin/bash -c "/var/lib/erpbizz/odoovenv/bin/python3 -m pip install -r /opt/release/requirements.txt"',
            f'su odoo -s /bin/bash -c "/var/lib/erpbizz/odoovenv/bin/python3 -m pip install /data/src/odoo_{VERSION}.{TSTAMP}.tar.gz"',
            'su odoo -s /bin/bash -c "createdb mycompany"',
            'su odoo -s /bin/bash -c "/var/lib/erpbizz/odoovenv/bin/erpbizz -d mycompany -i base --stop-after-init"',
            'su odoo -s /bin/bash -c "/var/lib/erpbizz/odoovenv/bin/erpbizz -d mycompany --pidfile=/data/src/erpbizz.pid"',
        ]
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/erpbizz (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su odoo -s /bin/bash -c "odoo -d mycompany -i base --pidfile=/data/src/erpbizz.pid"',
        ]
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        rpmbuild_dir = '/var/lib/erpbizz/rpmbuild'
        build_date = datetime.now().strftime('%a %b %d %Y')
        cmds = [
            'cd /data/src',
            'mkdir -p dist',
            'rpmdev-setuptree -d',
            f'cp -a /data/src/setup/rpm/erpbizz.spec {rpmbuild_dir}/SPECS/',
            f'tar --transform "s/^\\./erpbizz-{VERSION}/" -c -z -f {rpmbuild_dir}/SOURCES/erpbizz-{VERSION}.tar.gz .',
            f'rpmbuild -bb --define="%version {VERSION}" --define "%release {TSTAMP}" --define "%build_date {build_date}" /data/src/setup/rpm/erpbizz.spec',
            f'mv {rpmbuild_dir}/RPMS/noarch/erpbizz*.rpm /data/src/dist/'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, f'odoo-rpm-build-{TSTAMP}')
        os.rename(glob('%s/dist/erpbizz-*.noarch.rpm' % self.args.build_dir)[0], '%s/odoo_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su odoo -s /bin/bash -c "odoo -c /etc/erpbizz/erpbizz.conf -d mycompany -i base --stop-after-init"',
            'su odoo -s /bin/bash -c "odoo -c /etc/erpbizz/erpbizz.conf -d mycompany --pidfile=/data/src/erpbizz.pid"',
        ]
...
class DockerWine(Docker):
    ...
    def build(self):
        ...
        container_python = '/var/lib/erpbizz/.wine/drive_c/odoobuild/WinPy64/python-3.12.3.amd64/python.exe'
