...
def get_odoo_server_url():
    """Get the URL of the linked ErpBizz database.

    :return: The URL of the linked ErpBizz database.
    :rtype: str or None
    """
...
def get_commit_hash():
    return subprocess.run(
        ['git', '--work-tree=/home/pi/erpbizz/', '--git-dir=/home/pi/erpbizz/.git', 'rev-parse', '--short', 'HEAD'],
...
def get_version(detailed_version=False):
    if IS_RPI:
        image_version = read_file_first_line('/var/erpbizz/iotbox_version')
    elif IS_WINDOWS:
...
def download_iot_handlers(auto=True, server_url=None):
    """Get the drivers from the configured ErpBizz server.
...
def path_file(*args):
    """Return the path to the file from IoT Box root or Windows ErpBizz
    server folder
...
def download_from_url(download_url, path_to_filename):
    ...
    (Example: 'C:\\Program Files\\ErpBizz\\downloaded_file.zip')
    """
    try:
...
def unzip_file(path_to_filename, path_to_extract):
    ...
    Example: unzip_file('C:\\Program Files\\ErpBizz\\downloaded_file.zip', 'C:\\Program Files\\ErpBizz\\new_folder'))
