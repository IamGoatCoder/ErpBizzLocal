...
def get_db_branch(server_url):
    ...
    :param server_url: The URL of the connected ErpBizz database.
    ...
    try:
        ...
        _logger.exception('Could not reach configured server to get the ErpBizz version')
        return None
    try:
...
def check_version_upgrades(local_branch, db_branch):
    ...
    :param db_branch: The git branch of the connected ErpBizz database (Ex: "saas-19.1" / "master" etc.)
    """
    try:
        ...
        subprocess.run(
            ['/home/pi/erpbizz/addons/iot_drivers/tools/upgrade_scripts/upgrade_trixie/upgrade_trixie.sh'], check=True,
        )
...
def check_git_branch(server_url=None):
    """Check if the local branch is the same as the connected ErpBizz DB and
    checkout to match it if needed.

    :param server_url: The URL of the connected ErpBizz database (provided by decorator).
    """
    if IS_TEST:
        return
    ...
    try:
        ...
        _logger.info("IoT Box git branch: %s / Associated ErpBizz db's git branch: %s", local_branch, db_branch)
...
def _ensure_production_remote():
    ...
    (https://github.com/erpbizz/erpbizz.git).
    """
    production_remote = "https://github.com/erpbizz/erpbizz.git"
...
def misc_migration_updates():
    ...
    if path_file('odoo', 'addons', 'hw_drivers').exists():
        ...
        subprocess.run(
            ['sed', '-i', 's|iot_drivers|hw_drivers,hw_posbox_homepage|g', '/home/pi/erpbizz.conf'], check=False
        )
