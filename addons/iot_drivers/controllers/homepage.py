...
class IotBoxOwlHomePage(http.Controller):
    ...
    def odoo_service_restart(self):
        ...
        return json.dumps({
            'status': 'success',
            'message': 'ErpBizz service restarted',
        })

    @route.iot_route('/iot_drivers/iot_logs', type='http', cors='*')
    def get_iot_logs(self):
        logs_path = "/var/log/erpbizz/erpbizz-server.log" if IS_RPI else Path().absolute().parent.joinpath('odoo.log')
    ...
    def log_levels(self):
        ...
        return json.dumps({
            'title': "ErpBizz's IoT Box - Handlers list",
    ...
    def _update_logger_level(self, logger_name, new_level, available_log_levels, handler_folder=False):
        """Update (if necessary) ErpBizz's configuration and logger to the given logger_name to the given level.
    ...
    def _get_iot_handler_logger(self, handler_name, handler_folder_name):
        """
        Get ErpBizz Iot logger given an IoT handler name
