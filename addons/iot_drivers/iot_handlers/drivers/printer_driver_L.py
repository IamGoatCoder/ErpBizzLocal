...
class PrinterDriver(PrinterDriverBase):
    ...
    def print_raw(self, data, action_unique_id=None):
        ...
        try:
            with self.cups_lock:
                job_id = self.conn.createJob(self.device_identifier, 'ErpBizz print job', {'document-format': CUPS_FORMAT_AUTO})
                self.conn.startDocument(self.device_identifier, job_id, 'ErpBizz print job', CUPS_FORMAT_AUTO, 1)
