        ...
        if proxy_error:
            ...
            if error_code == 'invalid_signature':
                raise AccountEdiProxyError(
                    error_code,
                    _("Failed to connect to ErpBizz Access Point server. This might be due to another connection to ErpBizz Access Point "
        ...
        if edi_mode == 'demo':
            ...
            if 'error' in response:
                if response['error'] == 'A user already exists with this identification.':
                    ...
                    # ErpBizz side. We are for now forced to check the error message.
