...
class TestController(HttpCase):
    ...
    def test_05_internal_link_preview(self):
        ...
        response_without_preview_name = self.url_open(
            ...
            data=json_safe.dumps({
                "params": {
                    "preview_url": _get_full_url(f"/erpbizz/users/{self.portal_user.id}"),
                }
            }),
        ...
        response_wrong_action = self.url_open(
            ...
            data=json_safe.dumps({
                "params": {
                    "preview_url": _get_full_url("/erpbizz/actionInvalid/1"),
                }
            }),
            headers=self.headers
        )
        ...
        response_wrong_record = self.url_open(
            ...
            data=json_safe.dumps({
                "params": {
                    "preview_url": _get_full_url("/erpbizz/users/9999"),
                }
            }),
            headers=self.headers
        )
        ...
        response_model_record = self.url_open(
            ...
            data=json_safe.dumps({
                "params": {
                    "preview_url": _get_full_url(f"/erpbizz/res.users/{self.portal_user.id}"),
                }
            }),
            headers=self.headers
        )
        ...
        response_abstract_model = self.url_open(
            ...
            data=json_safe.dumps({
                "params": {
                    "preview_url": _get_full_url("/erpbizz/mail.thread/1"),
                }
            }),
            headers=self.headers
        )
