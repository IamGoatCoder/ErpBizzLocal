...
odoo_help() {
  echo '-------------------------------'
  echo ' Welcome to ErpBizz IoT Box tools'
  echo '-------------------------------'
  echo ''
  echo 'odoo                  Starts/Restarts ErpBizz server manually (not through odoo.service)'
  echo 'odoo_logs             Displays ErpBizz server logs in real time'
  echo 'odoo_conf             Displays ErpBizz configuration file content'
  echo 'install               Bypasses ramdisks to allow package installation'
  echo 'blackbox              Lists all serial connected devices'
  echo 'odoo_start            Starts ErpBizz service'
  echo 'odoo_stop             Stops ErpBizz service'
  echo 'odoo_restart          Restarts ErpBizz service'
  echo 'odoo_dev <branch>     Resets ErpBizz on the specified branch from odoo-dev repository'
  echo 'odoo_origin <branch>  Resets ErpBizz on the specified branch from the odoo repository'
  echo 'devtools              Enables/Disables specific functions for development (more help with devtools help)'
  echo ''
  echo 'ErpBizz IoT online help: <https://www.odoo.com/documentation/latest/applications/general/iot.html>'
}
...
# This will be modified by a unique password on the first start of ErpBizz
...
# Create ErpBizz user for odoo service and disable password login
