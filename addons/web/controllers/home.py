...
class Home(http.Controller):
    ...
    def index(self, s_action=None, db=None, **kw):
        if request.db and request.session.uid and not is_user_internal(request.session.uid):
            return request.redirect_query('/erpbizz/login_successful', query=request.params)
        return request.redirect_query('/erpbizz', query=request.params)

    def _web_client_readonly(self, rule, args):
        return False

    # ideally, this route should be `auth="user"` but that don't work in non-monodb mode.
    @http.route(['/web', '/erpbizz', '/erpbizz/<path:subpath>', '/scoped_app/<path:subpath>'], type='http', auth="none", readonly=_web_client_readonly)
    def web_client(self, s_action=None, **kw):

        # Ensure we have both a database and a user
        ensure_db()
        if not request.session.uid:
            return request.redirect_query('/erpbizz/login', query={'redirect': request.httprequest.full_path}, code=303)
        if kw.get('redirect'):
            return request.redirect(kw.get('redirect'), 303)
        if not security.check_session(request.session, request.env, request):
            raise http.SessionExpiredException("Session expired")
        if not is_user_internal(request.session.uid):
            return request.redirect('/erpbizz/login_successful', 303)
        ...
        try:
            ...
            return request.redirect('/erpbizz/login?error=access')

    @http.route(['/web/webclient/load_menus', '/erpbizz/webclient/load_menus'], type='http', auth='user', methods=['GET'], readonly=True)
    ...
    @http.route(['/web/login', '/erpbizz/login'], type='http', auth='none', readonly=False, list_as_website_content=_lt("Login"))
    ...
    @http.route(['/web/login_successful', '/erpbizz/login_successful'], type='http', auth='user', website=True, sitemap=False)
    ...
    @http.route(['/web/become', '/erpbizz/become'], type='http', auth='user', sitemap=False, readonly=True)
    ...
    @http.route(['/web/health', '/erpbizz/health'], type='http', auth='none', save_session=False)
