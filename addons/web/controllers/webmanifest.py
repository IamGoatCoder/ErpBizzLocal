...
class WebManifest(http.Controller):
        ...
        for module in module_ids:
            ...
            if data:
                shortcuts.append({
                    ...
                    'url': '/erpbizz?menu_id=%s' % data.mapped('res_id')[0],
    ...
    @http.route(['/web/manifest.webmanifest', '/erpbizz/manifest.webmanifest'], type='http', auth='public', methods=['GET'], readonly=True)
    ...
    @http.route(['/web/service-worker.js', '/erpbizz/service-worker.js'], type='http', auth='public', methods=['GET'], readonly=True)
    def service_worker(self):
        response = request.make_response(
            self._get_service_worker_content(),
            [
                ('Content-Type', 'text/javascript'),
                ('Service-Worker-Allowed', '/erpbizz'),
            ]
        )
        return response

    def _get_service_worker_content(self):
        """ Returns a ServiceWorker javascript file scoped for the backend (aka. '/erpbizz')
        """
    ...
    def scoped_app(self, app_id, path='', app_name=''):
        ...
        scoped_app_values = {
            'app_id': app_id,
            'apple_touch_icon': '/web/static/img/logo-ios.png',
            'app_name': app_name,
            'path': path,
    ...
    def scoped_app_manifest(self, app_id, path, app_name=''):
        """ Returns a WebManifest dedicated to the scope of the given app. A custom scope and start
            url are set to make sure no other installed PWA can overlap the scope (e.g. /erpbizz)
        """
        path = unquote(path)
        app_name = unquote(app_name) if app_name else self._get_scoped_app_name(app_id)
        webmanifest = {
