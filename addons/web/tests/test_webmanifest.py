...
class WebManifestRoutesTest(HttpCaseWithUserDemo):
    ...
    def test_webmanifest(self):
        ...
        data = response.json()
        self.assertEqual(data["name"], "ErpBizz")
        self.assertEqual(data["scope"], "/erpbizz")
        self.assertEqual(data["start_url"], "/erpbizz")
        ...
        for shortcut in data["shortcuts"]:
            ...
            self.assertTrue(shortcut["url"].startswith("/erpbizz?menu_id="))
    ...
    def test_serviceworker(self):
        ...
        self.assertEqual(response.headers["Service-Worker-Allowed"], "/erpbizz")

    def test_offline_url(self):
        """
        This route returns the offline page
        """
        response = self.url_open("/erpbizz/offline")
        self.assertEqual(response.status_code, 200)
    ...
    def test_apple_touch_icon(self):
        ...
        response = self.url_open("/web/static/img/logo-ios.png")
        self.assertEqual(response.status_code, 200)
