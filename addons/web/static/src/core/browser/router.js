...
export function startUrl() {
    return isScopedApp() ? "scoped_app" : "erpbizz";
}
...
function urlToState(urlObj) {
    ...
    if (["erpbizz", "scoped_app"].includes(prefix)) {
        ...
        if (prefix === "scoped_app" && !isDisplayStandalone()) {
            // make sure /scoped_app are redirected to /erpbizz when using the browser instead of the PWA
            const url = browser.location.origin + router.stateToUrl(state);
            urlObj.href = url;
        }
    }
 ...
 * external website). Unfortunately, ErpBizz wasn't designed to be compatible with this cache, which
...
browser.addEventListener("click", (ev) => {
    ...
    if (href && !href.startsWith("#")) {
        ...
        if (
            ...
            browser.location.pathname.startsWith("/erpbizz") &&
            (["/web", "/erpbizz"].includes(url.pathname) || url.pathname.startsWith("/erpbizz/")) &&
            a.target !== "_blank"
        ) {
            ev.preventDefault();
            state = router.urlToState(url);
            if (url.pathname.startsWith("/erpbizz") && url.hash) {
